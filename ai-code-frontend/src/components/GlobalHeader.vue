<template>
  <div class="global-header">
    <RouterLink class="brand" :to="getUserHome(loginUserStore.loginUser)" aria-label="AI Code 首页">
    ...
    <UserAccountMenu class="user-area" />
  </div>
</template>
...
import { getVisibleNavigation, navigationItems, type NavigationItem } from '@/config/menu'
import { useLoginUserStore } from '@/stores/loginUser'
import UserAccountMenu from '@/components/UserAccountMenu.vue'
import { getUserHome } from '@/access'
...
const visibleItems = computed(() => getVisibleNavigation(loginUserStore.loginUser, props.items))

// 优先匹配最长路径，子页面仍高亮所属菜单；根路径只匹配首页。
const selectedKeys = computed(() => {
  const activeItem = [...visibleItems.value]
